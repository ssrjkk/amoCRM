...
class TestContactsCRUD:
    ...
    def test_create_contact(self, api_client):
        resp = api_client.contacts.create({"name": "API Test Contact"})
        assert resp.status_code in [200, 201, 401, 402]

    def test_get_contact_not_found(self, api_client):
        resp = api_client.contacts.get(999999999)
        assert resp.status_code in [404, 401, 402]

    def test_create_and_update_contact(self, api_client):
        create_resp = api_client.contacts.create({"name": "Update Test"})
        ...
        update_resp = api_client.contacts.update(contact_id, {"name": "Updated Name"})

        assert update_resp.status_code in [200, 204, 401, 402]
...
class TestCompaniesCRUD:
    ...
    def test_create_company(self, api_client):
        resp = api_client.companies.create({"name": "API Test Company"})
        assert resp.status_code in [200, 201, 401, 402]

    def test_update_company(self, api_client):
        create_resp = api_client.companies.create({"name": "Test Company"})
        ...
        update_resp = api_client.companies.update(company_id, {"name": "Updated Company"})
...
class TestLeadsCRUD:
    ...
    def test_create_lead(self, api_client):
        resp = api_client.leads.create({"name": "Test Lead", "price": 50000})
        assert resp.status_code in [200, 201, 401, 402]
    ...
    def test_update_lead_status(self, api_client):
        create_resp = api_client.leads.create({"name": "Status Test"})
        ...
        update_resp = api_client.leads.update(lead_id, {"status_id": 142})
...
class TestTasksCRUD:
    ...
    def test_create_task(self, api_client):
        resp = api_client.tasks.create({"name": "Test Task", "task_type_id": 1, "entity_type": "leads", "entity_id": 1})
        assert resp.status_code in [200, 201, 401, 402, 422]
